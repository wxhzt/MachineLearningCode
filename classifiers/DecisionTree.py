...
class DecisionTree:
    ...
    def _calcEntropy(self,y):
        """
        计算熵
        ：param: 数据集的标签 array-like, shape = [n_sample,]
        """
        num = y.shape[0]
    ...
    def _getSubset(self,X,y,index,value):
        """
        根据属性获取子数据集,返回数据集中特征下标为index，特征值等于value的子数据集
        :param X: 特征向量 array-like， shape = [n_samples, n_features]
        :param y: 数据集的标签 array-like, shape = [n_sample,]
        """
        ret = []
        featVec = X[:,index]
    ...
    def _splitDataSetByID3(self,X,y):
        """ID3
        ...
        for i in range(numFeatures):
            ...
            for value in uniqueVals:
                sub_X,sub_y = self._getSubset(X,y,i,value)
    ...
    def _splitDataSetByC45(self,X,y):
        """C4.5
    ...
    def _createTree(self,X,y,featureIndex):
        ...
        #可以继续分割的话，确定最佳分割特征
        if self._mode == 'C4.5':
            bestFeatIndex = self._splitDataSetByC45(X,y)
        elif self._mode == 'ID3':
            bestFeatIndex = self._splitDataSetByID3(X,y)
        ...
        for value in uniqueVals:
            #对每个value递归地创建树
            sub_X,sub_y = self._getSubset(X,y, bestFeatIndex, value)
